def count_peaks_valleys(price_action: list[int]) -> tuple[int, int]:
    ...
    for price_idx in range(1, len(price_action) - 1):
        if price_action[price_idx - 1] < price_action[price_idx] > price_action[price_idx + 1]:
            peaks += 1
        elif price_action[price_idx - 1] > price_action[price_idx] < price_action[price_idx + 1]:
            valleys += 1
